...
int test_basic() {
  ...
  pointproofs_pp_bytes_dynamic pointproofs_pp_string_dynamic;
  pointproofs_vp_bytes_dynamic pointproofs_vp_string_dynamic;
  pointproofs_pp pp_recover;
  pointproofs_vp vp_recover;
  pointproofs_pp_bytes_dynamic pointproofs_pp_string_recover_dynamic;
  pointproofs_vp_bytes_dynamic pointproofs_vp_string_recover_dynamic;

  assert(pointproofs_pp_serial_dynamic(pointproofs_param.prover, &pointproofs_pp_string_dynamic) == 0);
  assert(pointproofs_vp_serial_dynamic(pointproofs_param.verifier, &pointproofs_vp_string_dynamic) == 0);
  assert(pointproofs_pp_deserial_dynamic(pointproofs_pp_string_dynamic, &pp_recover) == 0);
  assert(pointproofs_vp_deserial_dynamic(pointproofs_vp_string_dynamic, &vp_recover) == 0);
  assert(pointproofs_pp_serial_dynamic(pp_recover, &pointproofs_pp_string_recover_dynamic) == 0);
  assert(pointproofs_vp_serial_dynamic(vp_recover, &pointproofs_vp_string_recover_dynamic) == 0);

#ifdef DEBUG
  hexDump("prover param (in bytes)", pointproofs_pp_string_dynamic.data, pointproofs_pp_string_dynamic.len);
  hexDump("prover param recovered (in bytes)", pointproofs_pp_string_recover_dynamic.data, pointproofs_pp_string_recover_dynamic.len);

  hexDump("verifier param (in bytes)", pointproofs_vp_string_dynamic.data, pointproofs_vp_string_dynamic.len);
  hexDump("verifier param recovered (in bytes)", pointproofs_vp_string_recover_dynamic.data, pointproofs_vp_string_recover_dynamic.len);
#endif

  assert(memcmp(pointproofs_pp_string_dynamic.data, pointproofs_pp_string_recover_dynamic.data, pointproofs_pp_string_recover_dynamic.len) == 0);
  assert(memcmp(pointproofs_vp_string_dynamic.data, pointproofs_vp_string_recover_dynamic.data, pointproofs_vp_string_recover_dynamic.len) == 0);

  // generate a commit
  pointproofs_commitment commit;
  pointproofs_commitment_bytes_dynamic commit_string_dynamic;
  pointproofs_commitment commit_recover;
  pointproofs_commitment_bytes_dynamic commit_string_recover_dynamic;

  assert(pointproofs_commit(pp_recover, values, n, &commit) == 0);
  assert(pointproofs_commit_serial_dynamic(commit, &commit_string_dynamic) == 0);
  assert(pointproofs_commit_deserial_dynamic(commit_string_dynamic, &commit_recover) == 0);
  assert(pointproofs_commit_serial_dynamic(commit_recover, &commit_string_recover_dynamic) == 0);

#ifdef DEBUG
  hexDump("commit (in bytes)", commit_string_dynamic.data, commit_string_dynamic.len);
  hexDump("commit recovered (in bytes)", commit_string_recover_dynamic.data, commit_string_recover_dynamic.len);
#endif

  assert(strcmp((const char*) commit_string_dynamic.data, (const char*) commit_string_recover_dynamic.data) == 0);

  pointproofs_proof proof;
  pointproofs_proof_bytes_dynamic proof_string_dynamic;
  pointproofs_proof proof_recover;
  pointproofs_proof_bytes_dynamic proof_string_recover_dynamic;

  for (counter = 0; counter < k; counter++) {
    // generate a proof
    assert(pointproofs_prove(pp_recover, values, n, counter, &proof) == 0);
    assert(pointproofs_proof_serial_dynamic(proof, &proof_string_dynamic) == 0);
    assert(pointproofs_proof_deserial_dynamic(proof_string_dynamic, &proof_recover) == 0);
    assert(pointproofs_proof_serial_dynamic(proof_recover, &proof_string_recover_dynamic) == 0);

#ifdef DEBUG
    hexDump("proof (in bytes)", proof_string_dynamic.data, proof_string_dynamic.len);
    hexDump("proof recovered (in bytes)", proof_string_recover_dynamic.data, proof_string_recover_dynamic.len);
#endif

    assert(strcmp((const char*) proof_string_dynamic.data, (const char*) proof_string_recover_dynamic.data) == 0);
  ...
  active_pointproofs_pp_free(pointproofs_pp_string_dynamic); //need to free.
  active_pointproofs_pp_free(pointproofs_pp_string_recover_dynamic); //need to free.
  active_pointproofs_vp_free(pointproofs_vp_string_dynamic); //need to free.
  active_pointproofs_vp_free(pointproofs_vp_string_recover_dynamic); //need to free.
  active_pointproofs_commit_free(commit_string_dynamic); //need to free.
  active_pointproofs_commit_free(commit_string_recover_dynamic); //need to free.
  active_pointproofs_proof_free(proof_string_dynamic); //need to free.
   active_pointproofs_proof_free(proof_string_recover_dynamic); //need to free.
